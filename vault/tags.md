...
| `ai/diffusion` | 拡散モデル全般（Diffusion Models、Flow Matching、スコアマッチング） |
| `ai/medical` | 医療 AI（診断補助、医療画像解析、創薬） |
| `media/3d` | 3D グラフィックス、メッシュ処理、モーションキャプチャ |
| `media/music` | 音楽・ストリーミング配信、音楽産業 |

### Industry

| タグ | 説明 |
|------|------|
| `industry/tech` | テック業界・企業動向・ビジネスニュース |
| `industry/finance` | 金融業界・フィンテック・投資・銀行 |

### Policy

| タグ | 説明 |
|------|------|
| `policy/regulation` | 規制・法制度・標準化・政策 |
...
| `infra/vm` | 仮想マシン、サンドボックス実行環境 |
| `infra/hardware` | ハードウェア、半導体、チップパッケージング |
| `infra/network` | ネットワーキング、VPN、DNS、トンネリング |
...
| `dev/react` | React フレームワーク、コンポーネント設計 |
| `dev/csharp` | C# / .NET 開発 |
| `dev/cpp` | C++ 開発、標準規格 |
| `dev/ruby` | Ruby / Ruby on Rails 開発 |
| `dev/testing` | テスト設計・QA・品質保証・テスト自動化 |
| `dev/c` | C 言語開発、標準ライブラリ、システムプログラミング |
...
| `tool/cursor` | Cursor AI コードエディタ |
| `tool/zed` | Zed テキストエディタ（AI ネイティブ・Rust 製）|
| `tool/gh-cli` | GitHub CLI（gh コマンド）および関連拡張 |
| `tool/mcp` | Model Context Protocol サーバー・クライアント |
| `tool/vscode` | Visual Studio Code および GitHub Copilot VS Code 拡張 |
| `tool/playwright` | Playwright ブラウザ自動化フレームワーク |
...
| `security/api` | API キーの流出・悪用・アクセス制御 |
| `security/incident` | セキュリティインシデント、データ漏洩、不正アクセス事件 |
| `security/malware` | マルウェア、情報窃取型マルウェア、ランサムウェア |
...
| `science/math` | 純粋数学、幾何学、位相数学、数理科学 |

### Crypto

| タグ | 説明 |
|------|------|
| `crypto/blockchain` | 暗号通貨、ブロックチェーン、Bitcoin |
